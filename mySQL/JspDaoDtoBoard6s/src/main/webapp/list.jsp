...
<%
	...
	//round는 반올림이기에 수가  일정함 ceil는 올리는걸로 수가 계속 오르게끔하는거 floor는 걍 버리는거고 0,,,1,1,1,1이런식될가능성 높음
	int totalBlock = (int)Math.ceil((double) totalPage / Board.PAGE_LINK_AMOUNT);	
	//페이지 링크 한 블럭에 보일 페이지 링크 갯수 = Board.PAGE_LINK_AMOUNT
	...
	//만약 블럭 마지막 페이지 번호가 전체 페이지 마지막 번호보다 큰경우에는 블럭 마지막
	// 페이지번호를 페이지 마지막 번호로 저장하는 예외 처리 하기
	...
	int prevPage = 0;
	int nextPage = 0;
	//🐿️🐿️🐿️{블럭 처리 - 6/9}.(이전/다음) 관련 계산 등 처리🐿️🐿️🐿️  t
...
<%				
	...
	//hasNext 는 등재되어있는 함수 대부분 다음 리스 옮길때 쓰임
	if(hasNext){
			if(searchWord==null){	// <1>일반 리스트
%>
