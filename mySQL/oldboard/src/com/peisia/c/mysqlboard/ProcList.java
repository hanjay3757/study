...
public class ProcList {
	...
	static public void run() {
		...
		while(true) {
			...
			DispBoard.titleList();
			String sql = "select * from "+Db.tableNameBoard+" where b_reply_ori is null limit "+startIndex+","+PER_PAGE;
			try {
				Cw.wn("전송한sql문:"+sql);
	...
	static public void searchList(String searchWord) {
		...
		while(true) {
			...
			startIndex = (currentPage - 1) * PER_PAGE;	// 페이지의 첫 인덱스를 계산해서 저장하기
			DispBoard.titleList();
			String sql = "select * from "+Db.tableNameBoard+" where b_reply_ori is null"
