...
public class ProcList {
	...
	static public void run() {
		...
		while (true) {
			...
			String sql = "select * from " + Db.tableNameBoard + " where b_reply_ori is null limit " + startIndex + ","
					+ PER_PAGE;
			try {
				Cw.wn("전송한sql문:" + sql);
	...
	static public void searchList(String searchWord) {
		...
		while (true) {
			...
			startIndex = (currentPage - 1) * PER_PAGE; // 페이지의 첫 인덱스를 계산해서 저장하기
			String sql = "select * from " + Db.tableNameBoard + " where b_reply_ori is null" + " and b_title like '%"
					+ searchWord + "%'" + " limit " + startIndex + "," + PER_PAGE;
			try {
				Cw.wn("전송한sql문:" + sql);
				Db.result = Db.st.executeQuery(sql);
				while (Db.result.next()) { // 결과를 하나씩 빼기. 더 이상 없으면(행 수가 끝나면) false 리턴됨.
